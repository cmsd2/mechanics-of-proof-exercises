...
theorem Int.ModEq.pow_three (h : a ≡ b [ZMOD n]) : a ^ 3 ≡ b ^ 3 [ZMOD n] := by
  ...
  use x * (a ^ 2 + b * a + b ^ 2)
  calc
    a ^ 3 - b ^ 3 = (a - b) * (a ^ 2 + b * a + b ^ 2) := by ring
    _ = n * x * (a ^ 2 + b * a + b ^ 2) := by rw[hx]
    _ = n * (x * (a ^ 2 + b * a + b ^ 2)) := by ring
...
example {a b c d: ℤ} (h1 : a ≡ b [ZMOD n]) (h2 : c ≡ d [ZMOD n]) : ¬(a ⧸ c ≡ b ⧸ d [ZMOD n]) := by
  sorry
...
example {k : ℤ} (hb : k ≡ 3 [ZMOD 5]) :
  ...
  calc
    (4 * k + k ^ 3 + 3) - (4 * 3 + 3 ^ 3 + 3)
      = (k - 3) * (4 + k ^ 2 + 3 * k + 3 * 3) := by ring
    _ = 5 * x * (4 + k ^ 2 + 3 * k + 3 * 3) := by rw[hx]
    _ = 5 * (x * (4 + k ^ 2 + 3 * k + 3 * 3)) := by ring
