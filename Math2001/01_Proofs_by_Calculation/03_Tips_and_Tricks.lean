...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1
      = (z ^ 2 - 2) ^ 2 + 3 * z ^ 2 - 3 - z * (z ^ 2 - 2) := by ring
    _ = (0) ^ 2 + 3 * z ^ 2 - 3 - z * (0) := by rw [h1]
    _ = 3 * z ^ 2 - 3 := by ring
    _ = 3 * (z ^ 2 - 2 + 2) - 3 := by ring
    _ = 3 * (0 + 2) - 3 := by rw [h1]
    _ = 3 := by ring
...
example {p : ℝ} (h1 : 5 * p - 3 = 3 * p + 1) : p = 2 :=
  calc
    p = 5 * (5 * p - 3 + 3) / 5 / 2 - 3 * p / 2 := by ring
    _ = 5 * (3 * p + 1 + 3) / 5 / 2 - 3 * p / 2 := by rw [h1]
    _ = 2 := by ring
...
example {a b : ℝ} (h1 : a + 2 * b = 4) (h2 : a - b = 1) : a = 2 :=
  calc
    a = ((a + 2 * b) + (a - b) * 2) / 3 := by ring
    _ = (4 + 1 * 2) / 3 := by rw [h1, h2]
    _ = 2 := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    t ^ 4 + 3 * t ^ 3 - 3 * t ^ 2 - 2 * t - 2
      = (t ^ 2 - 4) ^ 2 + 5 * t ^ 2 - 18 + 3 * t * (t ^ 2 - 4) + 10 * t := by ring
    _ = (0)^2 + 5 * t ^ 2 - 18 + 3 * t * 0 + 10 * t := by rw [ht]
    _ = 5 * (t ^ 2 - 4) + 10 * t + 2 := by ring
    _ = 5 * 0 + 10 * t + 2 := by rw [ht]
    _ = 10 * t + 2 := by ring

example {x y : ℝ} (h1 : x + 3 = 5) (h2 : 2 * x - y * x = 0) : y = 2 :=
  calc
    y = y - (2 * x - y * x) / 2 + (2 * x - y * x) / 2 := by ring
    _ = y - 0 / 2 + (2 * x - y * x) / 2 := by rw [h2]
    _ = y * (2 - x) / 2 + x := by ring
    _ = y * (2 - (x + 3) + 3) / 2 + (x + 3 - 3) := by ring
    _ = y * (2 - 5 + 3) / 2 + (5 - 3) := by rw [h1]
    _ = 2 := by ring
...
example {p q r : ℚ} (h1 : p + q + r = 0) (h2 : p * q + p * r + q * r = 2) :
  ...
  calc
    p ^ 2 + q ^ 2 + r ^ 2
      = p ^ 2 + q ^ 2 + r ^ 2 + (p + q + r) ^ 2 - (p + q + r) ^ 2 := by ring
    _ = p ^ 2 + q ^ 2 + r ^ 2 + 0 ^ 2 - (p + q + r) ^ 2 := by rw [h1]
